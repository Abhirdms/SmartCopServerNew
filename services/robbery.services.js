...
// const Robbery = require('../model/robbery'); // Your Mongoose model for MongoDB
...
//           const result = await Robbery.create(results);
...
//         // Save data to the database (assumes Mongoose schema is defined)
//         const result = await Robbery.create(transformedResults);
...
const fs = require('fs');
const xlsx = require('xlsx');
const Robbery = require('../model/robbery'); // Keeping the original model name
    ...
    return new Promise(async (resolve, reject) => {
      ...
      function parseTimeToExcelSerialNumber(serialNumber) {
        ...
        for (let i = headerRowIndex + 1; i < rows.length; i++) {
          ...
          const result = await Robbery.create(rowData);
        ...
        const data = req.body;
          const result = await Robbery.create(data);
